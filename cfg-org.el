...
(defun org-toggle-office ()
  ...
  (setq org-refile-files (append (list (concat org-path-home "capture.org")
                                       (concat org-path-home "world.org")
                                       (concat org-path-home "home/new-words.org"))
                                 (file-expand-wildcards (concat org-path-work "gtd/*.org"))
                                 (file-expand-wildcards (concat org-path-work "gtd/*/*.org"))
                                 (file-expand-wildcards (concat org-path-home "gtd/kimi.org"))))
...
(setq org-capture-templates
      '(("t" "Inbox Task" entry (file+headline (concat org-path-home "capture.org") "Inbox")
        ...
        ("h" "Habit - 好的行為習慣" entry (file+headline (concat org-path-home "world.org") "Habit")
         "* %?  :habit:\n/Entered on %U/ \\\\")
        ("w" "Work Journal - 工作中的感悟" entry (file+datetree (concat org-path-work "journal/work-journal.org"))
		 "* %?\n  %i")
        ("m" "Meeting Journal - 會議紀要和討論結果" entry (file+datetree (concat org-path-work "journal/meeting-journal.org"))
		 "* %?")
        ("a" "R&D Journal" entry (file+datetree (concat org-path-work "journal/lab-journal.org"))
		 "* %?")
