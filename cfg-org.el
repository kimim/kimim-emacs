...
(defun org-toggle-office ()
  ...
  (setq org-agenda-files (list (concat org-gtd-path "office/")
                               (concat org-gtd-path "office/projects/")
                               (concat org-gtd-path "world.org")
                               (concat org-gtd-path "capture.org")))
  (setq org-refile-files (append (list (concat org-gtd-path "capture.org")
                                       (concat org-gtd-path "world.org")
                                       (concat org-gtd-path "home/new-words.org"))
                                 (file-expand-wildcards (concat org-gtd-path "office/*.org"))
                                 (file-expand-wildcards (concat org-gtd-path "office/*/*.org"))
                                 (file-expand-wildcards (concat org-gtd-path "home/kimi.org"))))
  (message "Agenda is from office..."))

(defun org-toggle-home ()
  (interactive)
	  (setq org-location-home-or-office "home")
      (setq org-agenda-files (list (concat org-gtd-path "home/")
                                   (concat org-gtd-path "world.org")))
      (setq org-refile-files (append (list (concat org-gtd-path "capture.org")
                                           (concat org-gtd-path "world.org")
                                           (file-expand-wildcards (concat org-gtd-path "home/*.org")))))
...
(setq org-capture-templates
      '(("t" "Inbox Task" entry (file+headline (concat org-gtd-path "capture.org") "Inbox")
		 "* %?\n/Entered on %U/ \\\\\ninspired by:  \\\\\n%a %K")
        ("o" "Inbox Actions" entry (file+headline (concat org-gtd-path "capture.org") "Inbox")
         "* OPEN %?\n/Entered on %U/ \\\\\ninspired by: %i")
        ("b" "Bugs" entry (file+headline (concat org-gtd-path "office/projects/prj-maint.org") "Maintenance")
         "* OPEN PRC:%?\n/Entered on %U/ \\")
        ("h" "Habit - 好的行為習慣" entry (file+headline (concat org-gtd-path "world.org") "Habit")
         "* %?  :habit:\n/Entered on %U/ \\\\\ninspired by: %i")
        ("w" "Work Journal - 工作中的感悟" entry (file+datetree (concat org-gtd-path "work-journal.org"))
		 "* %?\n  %i")
        ("m" "Meeting Journal - 會議紀要和討論結果" entry (file+datetree (concat org-gtd-path "meeting-journal.org"))
		 "* %?")
        ("a" "R&D Journal" entry (file+datetree (concat org-gtd-path "lab-journal.org"))
		 "* %?")
        ("n" "New word" entry (file+headline (concat org-gtd-path "home/new-words.org") "new-words")
		 "* %?\n")))
