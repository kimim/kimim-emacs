...
(defun kimim/frame-and-font-mini ()
  (interactive)
  ;; top, left ... must be integer
  (let ((width  (ceiling (/ (display-pixel-width) 2.5)))
        (height (/ (display-pixel-height) 2))
        (frame (selected-frame)))
    (set-frame-position frame (ceiling (* 1.5 width)) height)
    (set-frame-height frame (/ (* 4 height)
                               (* 5 (frame-char-height))))
    (set-frame-width frame (/ (* 4 width)
                              (* 5 (frame-char-width))))
...
(use-package custom
  ...
  :custom (;; do not warning when load new theme
           ...
           (custom-theme-directory
            "~/kimim-emacs/site-lisp/emacs-hippo-theme")))
#+end_src
...
(use-package pdf-tools
  ...
  :bind (:map pdf-view-mode-map
         ...
         ("k" . (lambda ()
                  ...
                  (interactive)
                  (image-next-line -1)))
         ("n" . kimim/pdf-view-next-page-command)
         ("p" . kimim/pdf-view-previous-page-command)
         ("<home>". (lambda ()
                      (interactive)
                      (image-scroll-up -999)))
         ("<end>". (lambda ()
  ...
  (defun kimim/pdf-view-next-page-command (&optional n)
    (interactive "p")
    (pdf-view-next-page-command n)
    (image-bob))

  (defun kimim/pdf-view-previous-page-command (&optional n)
    (interactive "p")
...
(use-package kimim
  ...
  (("C-x m y" . kimim/mail-attach-files)
   ...
   ("<pause>" . (lambda ()
                  (interactive)
                  (kimim/switch-to-scratch-buffer)
                  (iconify-frame)))
