...
(use-package olivetti
  ...
  :custom (olivetti-body-width (+ fill-column 12))
  :hook ((text-mode elfeed-show-mode) . olivetti-mode))
#+end_src
...
(use-package org
  ...
  (defun org--create-inline-image-advice (im)
    (nconc im (list :background "white")))
  ...
  (use-package org
    ...
    (defun jekyll-post ()
      ...
      (let ((category (jekyll-get-category))
        ...
        (org-publish-current-file nil t)
...
(use-package org-ref
  ...
  '((t . "${=key=:20} ${author:36} ${title:*} ${year:4} ${=has-pdf=:1}${=has-note=:1} ${=type=:7}")))
