...
(use-package sdcv
  ...
  (setq sdcv-dictionary-data-dir (file-truename "~/.emacs.d/dict"))
    ...
    (and (> (point) (save-excursion (back-to-indentation) (point)))
         (let ((string (buffer-substring (point) (max (line-beginning-position) (- (point) 80)))))
           (and (string-match-p " $" string)
                (not (string-match-p "[\x2a\x2b\x2d] $" string)) ;; -+* bullet items
...
(use-package eglot
  ...
  :hook
  ((c-mode c++-mode clojure-mode rust-mode) . eglot-ensure)
  (eglot-managed-mode
...
Keep several agenda buffers by turning on ~org-agenda-sticky~, and
rebind ~g~ in ~org-aganda-mode-map~ to redo all agenda buffers.

Bind ~C-c j~ to ~kimim/org-agenda-planning~ to open agenda list and todo
list for planning side by side.

#+begin_src emacs-lisp
(use-package org
  :bind
  (("C-c j" . kimim/org-agenda-today)
   :map
   org-agenda-mode-map
   ("g" . kimim/org-agenda-redo-all-buffers)
   ("M-j" . kimim/org-agenda-today)
   ("M-p" . kimim/org-agenda-planning)
   ("M-q" . kimim/org-agenda-quit-all-agenda-buffers))
  :custom
  (org-agenda-sticky t)

  :config
  (defun kimim/org-agenda-redo-all-buffers ()
    "Redo all agenda buffers."
    (interactive)
    (org-agenda-redo-all t))

  (defun kimim/org-agenda-today ()
    "Open todo list and agenda list for task in today."
    (interactive)
    (org-agenda nil "t")
    (org-agenda-list nil))

  (defun kimim/org-agenda-planning ()
    "Open todo list and agenda list for task planning."
    (interactive)
    (org-agenda nil "#")
    (org-agenda-list nil))

  (defun kimim/org-agenda-quit-all-agenda-buffers ()
    "Quit all buffers in `org-agenda-mode'."
    (interactive)
    (let (blist)
