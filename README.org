...
(use-package celestial-mode-line
  ...
  (defun kimim/update-theme-with-sun (orig-fun &optional date)
    (let ((sun-before (and (< 0 (length celestial-mode-line-string))
                           (substring
                            (substring-no-properties
                             celestial-mode-line-string 0 -5)
                            -2 nil))))
      (apply orig-fun date)
      (let ((sun-after (substring
                        (substring-no-properties
                         celestial-mode-line-string 0 -5)
                        -2 nil)))
        (cond
         ((and (or (not sun-before)
                   (equal sun-before
                          (cdr (assoc
                                'sunrise
                                celestial-mode-line-sunrise-sunset-alist))))
  ...
  ;;(advice-remove 'celestial-mode-line-update #'kimim/update-theme-with-sun)
...
(use-package org
  :bind
  (("C-c j" . kimim/org-agenda-planning)
   :map
   org-agenda-mode-map
