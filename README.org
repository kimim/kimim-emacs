...
(defun kimim/frame-and-font ()
  ...
  (when window-system
    ...
    (let* ((width (ceiling (* 0.8 (kimim/screen-width))))
      ...
      (kimim/menu-and-bar)
      ;; avoid WSL frame size issue when using Win-Left and Win-Right
      ;; if we set frame alist, after Win-Left, there is a gap between
      ;; frame and screen bottom.
      (when (not (running-in-wsl-p))
...
(bind-key "C-x f" 'toggle-frame-maximized)
#+end_src
  ...
  (advice-add
   ...
   (lambda ()
     (set-face-attribute
      ...
      :underline (face-attribute 'mode-line :background))))
...
(use-package kimim
  ...
  (("<f9>" . kimim/xterm)
   ...
   ("C-c s" . kimim/outlook-send-buffer-file)
   ...
   ("<f6>" . (lambda() (interactive)
               (if (s-contains? "temp/scratch.org" (buffer-file-name))
                   (delete-window)
                 (find-file-other-window "~/temp/scratch.org"))))
   ("<f7>" . bury-buffer)
   ("<f8>" . unbury-buffer)
   ...
   ("C-x y" . kimim/wsl-paste-from-clipboard)
...
(use-package fanyi
  ...
  (defun kimim/fanyi-dwim-local ()
    (interactive)
    (setq-local fanyi-providers nil)
    (call-interactively #'fanyi-dwim))
...
(use-package ediff
  ...
  :config
  ;; https://emacs.stackexchange.com/questions/9322
...
** auto-revert
Automatically update new log items in log file:
...
(use-package image-mode
  ...
  (:map image-mode-map
        ...
        ("k" . (lambda ()(interactive) (image-previous-line 10)))))
#+end_src
* Search and Finding
...
(use-package dired
  ...
  (dired-dwim-target t)
  ;; show only size, month, day, year and file name
  (dired-hide-details-preserved-columns '(3 4 5 6 7))
  :hook (dired-mode . dired-hide-details-mode)
  :bind
  (("C-x C-j" . dired-jump)
   :map dired-mode-map
   ("C-c l" . kimim/dired-get-org-link)
   ("C-c s" . kimim/dired-outlook-send)
  ...
  (define-advice dired-next-line (:after (arg) jump-min)
    "Jump to the beginning when at the end"
    (when (eobp) (goto-char (point-min))))

  (define-advice dired-previous-line (:after (arg) jump-max)
    "Jump to the end when at the beginning"
    (when (bobp) (goto-char (point-max)) (dired-previous-line 1)))
  ...
  (defun kimim/dired-outlook-send ()
    "Send marked Dired files as Outlook attachments from WSL2."
    (interactive)
    (unless (derived-mode-p 'dired-mode)
      (error "This command must be run in Dired"))
             ...
             (lambda (f)
               (string-trim
                (shell-command-to-string
                 (format "wslpath -w %s" (shell-quote-argument f)))))
             files))
           (ps-script
            (concat
...
** copilot
Copilot is an AI-powered code completion tool that can suggest code
snippets. It can also be used for writing comments and documentation.
...
(use-package copilot
  ...
  (add-to-list 'copilot-indentation-alist '(plantuml-mode 4)))
#+end_src
...
(use-package project
  :bind (("C-x p r" . project-find-ripgrep-regexp)
         ...
         ("C-x p ;" . project-recompile)
         ("C-x p f" . (lambda()
                        (interactive)
                        (project-find-file t))))
  :custom (vc-directory-exclusion-list
...
(use-package magit
  ...
  :config
  (defun kimim/kill-magit-buffers ()
    "Kill all Magit related buffers."
    (interactive)
    (let ((magit-buffers (seq-filter (lambda (buf)
...
(use-package lsp-mode
  ...
  :functions lsp--expand-snippet
  :commands lsp
  :custom
  (lsp-headerline-breadcrumb-icons-enable t)
  (lsp-headerline-breadcrumb-enable t)
  ;; red squiggly lines are ugly
  ...
  :bind (:map
         ...
         ("C-x l d" . lsp-describe-thing-at-point)
...
(use-package plantuml-mode
  ...
  (defun kimim/plantuml-preview-on-save ()
    "Re-generate PlantUML preview after saving."
    (when (derived-mode-p 'plantuml-mode)
      (plantuml-preview 0)))

  (add-hook 'plantuml-mode-hook
            (lambda () (add-hook 'after-save-hook
                                 #'kimim/plantuml-preview-on-save
                                 nil 'local)))
        ...
        (with-current-buffer buf
          ...
          (when (and (equal "svg" output-type))
            (let ((inhibit-read-only t)
              ...
              ;; do not erase buffer, otherwise the image will disappear
              ;;(erase-buffer)
              (insert-image (create-image
                             svg-data 'svg t
                             :background plantuml-svg-background))))))
      (set-window-point (get-buffer-window buf 'visible) (point-min))
      ;; do not select the preview window
      ;; (let ((bufw (get-buffer-window "*PLANTUML Preview*")))
      ;;   (when bufw
      ;;     (select-window bufw))))
      )))
#+end_src
...
(use-package chatu
  ...
  :config
  (setq chatu-drawio-empty
      "<mxfile>
  <diagram>
    <mxGraphModel pageWidth=\"1000\" pageHeight=\"560\">
      <root>
      </root>
    </mxGraphModel>
  </diagram>
...
(use-package org
  ...
  (:map org-mode-map
   ...
   ("M-h" . nil)
   ("C-'" . org-emphasize)
   ("C-c C-v p" . kimim/preview-babel-image)
  ...
  (define-advice org-agenda-next-line (:before nil jump-min)
    "Jump to the beginning when at the end"
    (when (eobp) (goto-char (point-min))))

  (define-advice org-agenda-previous-line (:before nil jump-max)
    "Jump to the end when at the beginning"
...
(use-package org
  ...
  (define-advice org-babel-execute-src-block
    ...
    (let* ((language (org-element-property
                      :language (org-element-at-point)))
           (language-to-load
            (if (string= language "C++")
                "C"
              language)))
      (unless (cdr (assoc (intern language-to-load)
                          org-babel-load-languages))
        (require (intern (concat "ob-" language-to-load)))
        (add-to-list 'org-babel-load-languages
                     (cons (intern language-to-load) t))
...
(use-package org
  ...
  (defun kimim/org-beamer-to-pdf-ppt-async ()
    (interactive)
    (setq-local org-latex-default-class "beamer")
    (org-beamer-export-to-pdf t))

  (defun kimim/org-latex-to-pdf-article-async ()
    (interactive)
...
(use-package ox-html
  ...
  (define-advice org-html-paragraph
      (:before (paragraph contents info)
               join-consecutive-chinese)
...
(use-package org
  ...
  ;; high light line in agenda mode to indicate the tags apart
  ...
  ;; (define-advice org-schedule (:after () add-todo)
  ...
  ;; (define-advice org-deadline (:after () add-todo)
  ;;   (if (or (string= "OPEN" (org-get-todo-state))
  ;;           (string= "WAIT" (org-get-todo-state))
  ;;           (string= "CLOSE" (org-get-todo-state)))
  ;;       (org-todo "WAIT")
