...
The solution is to use =winpty= [fn:8]
#+BEGIN_SRC sh
$ winpty C:/kimikit/emacs/bin/emacs.exe -nw
#+END_SRC

Installation of =winpty=:
#+BEGIN_SRC sh
$ ./configure
...
install -m 755 -p -s build/winpty.exe /usr/local/bin
install -m 755 -p -s build/winpty.dll /usr/local/bin
install -m 755 -p -s build/winpty-agent.exe /usr/local/bin
mkdir -p /usr/local/bin
install -m 755 -p -s build/winpty-debugserver.exe /usr/local/bin
mkdir -p /usr/local/lib
install -m 644 -p build/winpty.lib /usr/local/lib
mkdir -p /usr/local/share/doc/winpty
install -m 644 -p LICENSE /usr/local/share/doc/winpty
install -m 644 -p README.md /usr/local/share/doc/winpty
install -m 644 -p RELEASES.md /usr/local/share/doc/winpty
mkdir -p /usr/local/include/winpty
install -m 644 -p src/include/winpty.h /usr/local/include/winpty
install -m 644 -p src/include/winpty_constants.h /usr/local/include/winpty
#+END_SRC
...
Use self defined color theme 2 seconds after init.
...
#+BEGIN_SRC emacs-lisp
  ...
  catergories and tags are generated from exisiting posts"
      ...
      (insert "]")
      (insert "\ntags: [")
      (insert (ivy-read "tags: " '("emacs" "org-mode" "c prog"
                                   "Deutsch" "Français" "English"
                                   "management")))
      (insert "]\n---\n#+END_EXPORT\n\n")
      )
...
[fn:8] https://github.com/rprichard/winpty
