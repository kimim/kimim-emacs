...
(use-package dired
  ...
  (define-advice dired-next-line (:after-while (arg) move-down-line)
  ...
  (define-advice dired-previous-line (:after-while (arg) move-up-line)
...
(use-package org
  ...
  (define-advice org-babel-execute-src-block (:around () load-language)
...
(use-package ox-html
  ...
  (define-advice org-html-paragraph
      (:before (paragraph contents info)
               activatefsh-org-html-paragraph-advice)
